...
import { useRoqPlatformUserProfiles, useLoanCount, useLoanOfficerCount, useVehicleCount } from 'lib/roq';

import { FinanceManagerListPage as ShowCaseList } from 'pages/finance-managers';

function DashboardPage() {
  const { session } = useSession();

  const loanStat = useLoanCount();

  const loanOfficerStat = useLoanOfficerCount();

  const vehicleStat = useVehicleCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Loans',
      result: loanStat?.data,
      isLoading: loanStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Loan Officers',
      result: loanOfficerStat?.data,
      isLoading: loanOfficerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Vehicles',
      result: vehicleStat?.data,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Finance Managers
                </Text>
                <NextLink href={`finance-managers`} passHref legacyBehavior>
                  <Button

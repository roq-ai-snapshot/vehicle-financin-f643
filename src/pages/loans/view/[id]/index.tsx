...
import { useLoanFindFirst, useRoqClient } from 'lib/roq';

import { LoanInterface } from 'interfaces/loan';
import { FinanceManagerListPage } from 'pages/finance-managers';
import { LoanOfficerListPage } from 'pages/loan-officers';
...
function LoanViewPage() {
  ...
  const queryParams = useMemo<{ include: { vehicle: boolean; user: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useLoanFindFirst(queryParams, {}, { disabled: !id });
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Interest Rate" text={data?.interest_rate} />

                <FormListItem label="Loan Term" text={data?.loan_term} />

                <FormListItem label="Monthly Payment" text={data?.monthly_payment} />

                <FormListItem label="Loan Status" text={data?.loan_status} />

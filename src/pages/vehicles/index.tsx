...
import { useRoqClient, useVehicleFindManyWithCount } from 'lib/roq';
...
export function VehicleListPage(props: VehicleListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: [, 'loan.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['make.contains', 'model.contains', 'color.contains', 'vin.contains'],
          ...params.filters,
        },
        'vehicle',
      ),
  ...
  const { data, error, isLoading, mutate } = useVehicleFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.vehicle.delete({ where: { id } });
      await mutate();
    } catch (error) {
